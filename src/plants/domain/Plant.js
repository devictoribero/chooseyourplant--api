class Plant {
  constructor() {
    ...
    this.mostCommonName = plant.mostCommonName;
    this.names = plant.names;
    ...
    this.images = getPlantImages(plant.images);
    this.videos = getPlantVideos(plant.videos);
    this.popularity = getPlantPopularity(plant);
    this.linksToBuy = getLinksToBuyPlant(plant.productLinks);
    this.cares = getPlantCares(plant);
    this.growth = getPlantGrowth(plant);
    this.features = getPlantFeatures(plant);
  }

  static getFoliageSize(size) {
    if (size === "mini") {
...
function getLinksToBuyPlant(linksToBuyPlant) {
  if (linksToBuyPlant && linksToBuyPlant.length === 0) {
    return [];
  }
